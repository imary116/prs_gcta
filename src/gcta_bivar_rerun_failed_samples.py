...
def greml(b, input, phenotype, preval1: float = 0.01, preval2: float = 0.01, thread_val: int = 10):
    ...
    # for bivariant use "--reml-bivar" and "--reml-bivar-prevalence" instead of "--reml" and "--prevalence"
    # --reml-maxit - increase the maximum number of iterations (default is 100)
    j.command(
        f'''/gcta/gcta_1.93.1beta/gcta64 --grm {input} --pheno {phenotype} --reml-bivar --reml-bivar-prevalence {preval1} {preval2} --reml-maxit 300 --out {j.ofile} --thread-num {thread_val}''')
    j.command(f'echo {j.ofile.log}')
    return j
...
if __name__ == '__main__':
    ...
    # failed samples: xclo3_xswe5, xclm2_xclo3, celso_cgs1c
    sample_names = ['xclo3_xswe5', 'xclm2_xclo3', 'celso_cgs1c']
    for samples in sample_names:
        b = hb.Batch(backend=backend, name=f'GCTA-bivariate: {samples}')  # define batch

        # read individual fam files for phenotype file
        file1 = b.read_input_group(
            bed=f'gs://imary116/prs/PLINK/input/{samples.split("_")[0]}/scz.{samples.split("_")[0]}.QC.bed',
            bim=f'gs://imary116/prs/PLINK/input/{samples.split("_")[0]}/scz.{samples.split("_")[0]}.QC.bim',
            fam=f'gs://imary116/prs/PLINK/input/{samples.split("_")[0]}/scz.{samples.split("_")[0]}.QC.fam')

        file2 = b.read_input_group(
            bed=f'gs://imary116/prs/PLINK/input/{samples.split("_")[1]}/scz.{samples.split("_")[1]}.QC.bed',
            bim=f'gs://imary116/prs/PLINK/input/{samples.split("_")[1]}/scz.{samples.split("_")[1]}.QC.bim',
            fam=f'gs://imary116/prs/PLINK/input/{samples.split("_")[1]}/scz.{samples.split("_")[1]}.QC.fam')

        # read in files with snps to keep
        snp_list1 = b.read_input(f'gs://imary116/prs/PLINK/input/{samples.split("_")[0]}/scz.{samples.split("_")[0]}.QC.prune.in')
        snp_list2 = b.read_input(f'gs://imary116/prs/PLINK/input/{samples.split("_")[1]}/scz.{samples.split("_")[1]}.QC.prune.in')

        # file1_size = hl.utils.hadoop_stat(file1.bed)['size'] # format is number + the letter 'G'

        # Step 1: merging
        run_merge = merge(b, file1, file2.bed, file2.bim, file2.fam)  # outputs from this run are merged bed, bim and fam - run_merge.ofile
        b.write_output(run_merge.ofile.log,f'gs://imary116/gcta/bivariate_run/output/log_files/merge_{samples}')  # write out log file

        # Step 2: standard GWAS QC
        run_GWAS_qc = GWAS_qc(b, run_merge.ofile)  # outputs from this run are QCed [merged] bed, bim and fam - run_GWAS_qc.ofile
        b.write_output(run_GWAS_qc.ofile.log, f'gs://imary116/gcta/bivariate_run/output/log_files/GWASqc_{samples}')  # write out log file

        # Step 3: phenotype file
        run_pheno = create_pheno(b, file1.fam, file2.fam, run_GWAS_qc.ofile.fam)  # output is a phenotype file - run_pheno.ofile
        ...
        b.write_output(run_grm.ofile.log, f'gs://imary116/gcta/bivariate_run/output/log_files/grm_{samples}')  # write out log file

        # Step 5: GCTA-GREML
        run_greml = greml(b, run_grm.ofile, run_pheno.ofile, preval1, preval2, thread_val)
        b.write_output(run_greml.ofile.log, f'gs://imary116/gcta/bivariate_run/output/log_files/greml_{samples}')  # write out log file

        # Step 6: Parse output
        run_parse = parse_output(b, run_greml.ofile)

        b.write_output(run_parse.ofile, f'gs://imary116/gcta/bivariate_run/output/{samples}.txt')  # write out final output
