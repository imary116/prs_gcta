...
def grm(b, input, maf_val: float = 0.01, thread_val: int = 10):
    ...
    j.command(
        f'''/gcta/gcta_1.93.1beta/gcta64 --bfile {input} --autosome --maf {maf_val} --make-grm --out {j.ofile} --thread-num {thread_val}''')
    j.command(f'echo {j.ofile.log}')
    return j
    ...
    for first_sample in sample_names:
        for second_sample in sample_names:
            if second_sample > first_sample:
                ...
                b.write_output(run_merge.ofile.log, f'gs://imary116/gcta/bivariate_run/output/log_files/merge_{first_sample}_{second_sample}')  # write out log file
                ...
                b.write_output(run_GWAS_qc.ofile.log, f'gs://imary116/gcta/bivariate_run/output/log_files/GWASqc_{first_sample}_{second_sample}')  # write out log file
                ...
                # Step 4: GCTA-GRM
                run_grm = grm(b, run_GWAS_qc.ofile, maf_val, thread_val)
                b.write_output(run_grm.ofile.log, f'gs://imary116/gcta/bivariate_run/output/log_files/grm_{first_sample}_{second_sample}')  # write out log file

                # Step 5: GCTA-GREML
                run_greml = greml(b, run_grm.ofile, run_pheno.ofile, preval1, preval2, thread_val)
                b.write_output(run_greml.ofile.log, f'gs://imary116/gcta/bivariate_run/output/log_files/greml_{first_sample}_{second_sample}')  # write out log file

                # Step 6: Parse output
                run_parse = parse_output(b, run_greml.ofile)

                b.write_output(run_parse.ofile, f'gs://imary116/gcta/bivariate_run/output/{first_sample}_{second_sample}.txt')  # write out final output
